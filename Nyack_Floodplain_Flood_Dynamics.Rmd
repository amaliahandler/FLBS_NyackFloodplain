...
# Standardize the y axis limits based on the sensor with the largest range of values for march through may
year      <- 2015
start     <- as.POSIXct(paste0(year, "-03-01"))
end       <- as.POSIXct(paste0(year, "-05-31"))
...
# Find the time of peak water level in main stem
temp <- level$`6`[level$`6`$datetime >= start & level$`6`$datetime <= end,]
...
# Make figure of 2015 data
# Add vertical bar for time of peak water level at CASC
max_diff <- max_diff_level
y_label <- "Level (m)"

png("./Figures/Flood Figures/03-2015_to_05-2015_GW Level.png", units = "in", 
    width = 16, height = 12, res = 100, bg = "transparent")
...
lapply(level, function(x){
  ...
  plot(temp$datetime, temp$level_m, xlab = " ", ylab = y_label, 
       xlim = c(start, end), xaxs = 'i', yaxs = 'i',
       cex.lab = 2, cex.axis = 2, pch = 20)
  
  abline(v = casc_time_max, lwd = 3)
...
# Find the max for all wells and compare to the time of peak in the mainstem
i <- 7
...
# Make a df to store the lag information
lag_hr <- as.numeric(difftime(max_time, casc_time_max))
...
lag_7 <- gw_lags("2014-06-01", "2014-06-30")
lag_8 <- gw_lags("2014-08-01", "2014-08-31")
...
lag_13 <- gw_lags("2016-05-18", "2016-05-22"); lag_13[2, "lag_hr"] <- NA
