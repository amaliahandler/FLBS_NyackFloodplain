...
gw_lags <- function(start_date, end_date){
  ...
  for(i in 1:7){
    l.temp <- level[[i]][level[[i]]$datetime >= start & level[[i]]$datetime <= end,]
    ini_lev <- l.temp$level_m[1]
    max_lev <- max(na.omit(l.temp$level_m))
    chg     <- max_lev - ini_lev
    ...
    c.temp <- cond[[i]][cond[[i]]$datetime >= start & cond[[i]]$datetime <= end,]
    ini_cond <- c.temp$cond[1]
    max_abs  <- max(abs(c.temp$cond))
    max_cond <- ifelse()
    }
...
# Figure of change in water level across wells and main stem
