...
for(i in 2013:2019){
  ...
  # Standardize the y axis limits based on the CASC sensor
  # Find the max and min of the water level data for CASC
  temp      <- do$`6`[do$`6`$datetime >= start & do$`6`$datetime <= end,]
  max_temp  <- max(temp$do_temp)
  min_temp  <- min(temp$do_temp)
  casc_diff <- (max_temp - min_temp) / 2
  ...
  lapply(do, function(x){
    ...
    ylim_up   <- (min_temp + mid_point) + casc_diff
    ylim_lo   <- (min_temp + mid_point) - casc_diff
...
for(i in 2013:2019){
  ...
  lapply(do, function(x){
    ...
    ylim_up   <- (min_sat + mid_point) + casc_diff
    ylim_lo   <- (min_sat + mid_point) - casc_diff
