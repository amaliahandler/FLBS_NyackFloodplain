...
gw_lags <- function(start_date, end_date){
  ...
  par(mfrow = c(8,1), oma = c(1,1,1,0), mar = c(1,5,0,2))
  ...
  plot(tmp$datetime, tmp$level_m, xlab = " ", ylab = "Level (m)", 
         xlim = c(start, end), xaxs = 'i', col = "purple",
         cex.lab = 2, cex.axis = 2, pch = 19, xaxt = 'n')
  mtext(paste(tmp[1,2], tmp[1,1], sep = '_'), side = 4,
          line = 1, cex = 1.5)
  ...
  lapply(cond, function(x){
    temp <- x[x$datetime >= start & x$datetime <= end,]
    plot(temp$datetime, temp$cond, xlab = " ", ylab = "Cond (mS)", 
         xlim = c(start, end), xaxs = 'i',
         cex.lab = 2, cex.axis = 2, pch = 19, xaxt = 'n')
    abline(v = casc_time_max, lwd = 2, col = "red")
    abline(v = min_cond_time, lwd = 2, col = "blue")
    mtext(paste(temp[1,2], temp[1,1], sep = '_'), side = 4,
          line = 1, cex = 1.5)
    box(lwd = 2)
    })
...
lag_1 <- gw_lags("2015-03-25", "2015-04-25")
