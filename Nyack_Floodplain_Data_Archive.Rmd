...
png("./Figures/DO_temp_anomaly_det_zero_rm.png", units = "in", 
    width = 12, height = 12, res = 300, bg = "transparent")

par(mfrow = c(5,2), oma = c(2,2,1,1), mar = c(3,5,2,0))

lapply(seq_along(gap_fill), function(i){
  # Plot raw data
  plot(gap_fill[[i]]$datetime, gap_fill[[i]]$do_temp, xlab = " ", ylab = y_label, 
       cex.lab = 2, cex.axis = 2, pch = 20, cex.pch = 2)

  # Plot interpolated data
  points(gap_fill[[i]]$datetime[gap_fill[[i]]$interp == 'yes'],
         gap_fill[[i]]$do_temp_interp[gap_fill[[i]]$interp == 'yes'],
         pch = 20, cex.pch = 2, col = 'blue')
  
  # Highlight detected anomalies
  points(anom_do_temp[[i]]$datetime[anom_do_temp[[i]]$anomaly == 'Yes'],
         anom_do_temp[[i]]$observed[anom_do_temp[[i]]$anomaly == 'Yes'],
         pch = 1, cex.pch = 2, col = 'red')
    
  title(paste(gap_fill[[i]][1,2], gap_fill[[i]][1,3], sep = '_'), 
        line = 0.5, cex.main = 2)
  box(lwd = 2)
