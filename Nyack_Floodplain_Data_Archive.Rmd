...
sensor <- c('01','01','02','02','03','04','05','07')
start  <- as.Date(c('2012-01-01','2017-01-01','2012-01-01','2017-04-01',
                    '2012-01-01','2016-01-01','2012-01-01','2012-01-01'))
end    <- as.Date(c('2016-02-01','2018-01-01','2016-01-01','2019-07-01',
                    '2016-01-01','2019-07-01','2015-03-01','2019-07-01'))
th     <- c(2.3,2.3,0.2,0.2,3.13,1.9,2.4,3.5)
...
for(i in 1:length(sensor)){
  ha_level_red <- ha_level_red[-which(ha_level_red$sensor_number == sensor[i] &
                       ...
                       ha_level_red$level_m <= th[i]), "level_m_flag"] <- 3
}


# Save the low data, but remove these row for the gap filling process.
secondary_low_data <- lapply(ha_level_list, function(x){
  sn <- x$sensor_number[1]
  if(sn %in% sensor){
    if(length(sensor[sensor == sn]) == 2){
      temp <- 
      s <- secondary_low_data[match(sn, secondary_low_data$sensor), 'start']
      e <- secondary_low_data[match(sn, secondary_low_data$sensor), 'end']
      x <- x[which(x$level_m <= 0 & x$datetime >= s & x$datetime <= e), ]
    } else {
      s <- secondary_low_data[match(sn, secondary_low_data$sensor), 'start']
      e <- secondary_low_data[match(sn, secondary_low_data$sensor), 'end']
      x <- x[which(x$level_m <= 0 & x$datetime >= s & x$datetime <= e), ]
