...
temp <- lapply(seq(ha_do_list), function(i){
  # Merge the anomaly data into the site data
  m <- merge(ha_do_list[[i]], do_temp_anom[[i]][, c('datetime', 'anomaly')], 
             by = 'datetime')
  
  # Create a flag column
  m$do_temp_flag[m$anomaly == 'Yes'] <- 9
...
nrow(compiled_do_data[which(compiled_do_data$flag == 9),])
...
anom_zeros <- lapply(do_v1, function(x){
...
temp <- lapply(seq(ha_do_list), function(i){
  ...
  # Create a flag column
  m$do_temp_flag[m$anomaly.x == 'Yes'] <- 9
  m$do_conc_flag[m$anomaly.y == 'Yes'] <- 9
