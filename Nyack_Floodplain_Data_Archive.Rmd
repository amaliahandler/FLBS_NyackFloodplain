...
# Store the glitches
sensor_glitch   <- ha_do_list[[9]][which((ha_do_list[[9]]$datetime >= glitch1_srt &
...
# Data to have a missing value in place of the "bad" data
#   sensor_service
#   sensor_glitch
#   cable_fault
# Data to be flagged as potentially anomalous
#   anom_zeros (temp and conc)
#   data flagged as anomalous from the anomalize package process
#   data from Casc that was collected while there was silt in the well
sensor_service <- sensor_service[, c('site', 'sensor_number', 'datetime', 'do_conc', 'do_temp')]

bad_data       <- rbind(sensor_service, sensor_glitch, cable_fault)

bad_data$do_conc      <- -999
bad_data$do_temp      <- -999
bad_data$do_conc_flag <- NA
bad_data$do_temp_flag <- NA

# Bind together the anomalous zero values
anom_zeros_temp$do_temp_flag  <- 1
anom_zeros_conc$do_conc_flag  <- 1

anom_zeros_temp$do_conc_flag  <- NA
anom_zeros_conc$do_temp_flag  <- NA

anom_zeros <- rbind(anom_zeros_temp, anom_zeros_conc)
...
compiled_do_data <- rbind(temp, anom_zeros, bad_data)
...
# How many flagged observations that are potentially anomalous?
nrow(compiled_do_data[which(compiled_do_data$do_temp_flag == 1 |
                            compiled_do_data$do_conc_flag == 1 |
                            compiled_do_data$do_conc_flag == 2),])

# How many observation are missing due to sensor serivcing, glitches, or other technical issues?
nrow(compiled_do_data[which(compiled_do_data$do_conc == -999),])

# Write the cleaned data to a csv file
# write.csv(compiled_do_data, './Data/HA_DO_QAQC.csv', row.names = FALSE, quote = FALSE)
...
png("./Figures/DO_conc_cleaned.png", units = "in", 
    width = 18, height = 12, res = 300, bg = "transparent")
...
lapply(vis, function(x){
  ...
  plot(x$datetime, x$do_conc, xlab = " ", ylab = y_label, cex.lab = 2, cex.axis = 2, pch = 20)
...
png("./Figures/DO_temp_cleaned.png", units = "in", 
    width = 18, height = 12, res = 300, bg = "transparent")
...
gap_fill <- lapply(gap_fill, function(x){
  # Interpolate data
  x$level_m_interp <- na_interpolation(x$level_m, option = "linear")
  
  # Column to identify interpolated values
  x$interp <- 'no'
  x$interp[is.na(x$level_m)] <- 'yes'
...
# Anomaly identification
# Run the anomaly detection process
anom_level_m <- lapply(seq_along(gap_fill), function(i){
  gap_fill[[i]] %>%  
    time_decompose(level_m_interp, 
                   method = "stl",
                   frequency = "24 hours",
                   trend = "2 weeks") %>%
    anomalize(remainder, 
...
# Merge the anomaly data into the gap_fill df
level_m_anom <- lapply(seq_along(gap_fill), function(i){
  gap_fill[[i]]$anomaly <- anom_level_m[[i]]$anomaly
  return(gap_fill[[i]])
}) 
...
png("./Figures/Water Level Figures/Water_level_alpha01.png", units = "in", 
    width = 12, height = 12, res = 300, bg = "transparent")

par(mfrow = c(5,2), oma = c(2,2,1,1), mar = c(3,5,2,0))

lapply(level_m_anom, function(x){
  # Plot raw data, without the interpolated data
  plot(x$datetime[x$interp == 'no'], x$level_m[x$interp == 'no'], 
       xlab = " ", ylab = y_label, cex.lab = 2, cex.axis = 2, pch = 20, cex.pch = 2)

  # Highlight detected anomalies in raw data (not including interpolated data)
  points(x$datetime[x$anomaly == 'Yes' & x$interp == 'no'],
         x$level_m[x$anomaly == 'Yes' & x$interp == 'no'],
         pch = 1, cex.pch = 2, col = 'red')
