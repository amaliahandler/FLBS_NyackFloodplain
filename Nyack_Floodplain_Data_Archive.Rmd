...
# Glitch that needs to be removed
xlims <- as.POSIXct(c('2014-06-27 15:00', '2014-06-28 15:00'))
plot(ha_water$datetime[ha_water$sensor_number == '01'],
     ha_water$do_temp[ha_water$sensor_number == '01'], xlim = xlims)

sensor_glitch <- ha_water[which(ha_water$sensor_number == '01' &
...
# Merge the do_temp data into the above list
gap_fill <- lapply(seq(gap_fill), function(i){
  merge(gap_fill[[i]], do_v2[[i]], by = "datetime", all.x = TRUE)
})
...
# Mark the period when CASC had a stilling well full of silt
...
compiled_do_data$do_conc_flag[which(compiled_do_data$sensor_number == '06' &
                                    compiled_do_data$datetime >= s &
                                    compiled_do_data$datetime <= e)] <- 2
...
# Most of the bad conductivity data comes from HA02, sensor 09, between 2019-07-20 and 2016-08-25. From the sensor log, it appears that it was serviced on 07-20, then on 08-25 it was serviced again and noted that the conductivity probe was out of the water apparently hung up. 
...
anom <- lapply(ha_cond_list, function(x){
  ...
  diff <- abs(x$cond[-1] - x$cond[1:(nrow(x)-1)])
...
test <- lapply(seq_along(ha_cond_list), function(i){
  ...
  ha_cond_list[[i]]$anomaly[1:length(anom[[i]])] <- anom[[i]]
...
png("./Figures/Cond Figures/Cond_val_diff_flag.png", units = "in", 
    width = 12, height = 12, res = 300, bg = "transparent")
...
# Find the percentile of observation that can be anomalous
cutoff <- lapply(seq_along(ha_cond_list), function(i){
  quan   <- 1 - num_anoms/nrow(ha_cond_list[[i]])
  cutoff <- as.numeric(quantile(differ[[i]], quan))
  return(cutoff)
})

# Flag all differences higher than the cutoff
flag <- NULL

lapply(seq_along(differ), function(i){
  for(n in 1:nrow(differ[[i]])){
    if(differ[[i]][n] >= cutoff[[i]]){
      flag[[i]][n] <- 'yes'
    } else {flag[[i]][n] <- 'no'}
  }
})
...
flagged <- lapply(seq_along(differ), function(i){
  for(element in 1:length(differ[[i]])){
      if(differ[[i]][element] >= cutoff[[i]]){
        flag[[i]][element] <- 'yes'
      } else {flag[[i]][element] <- 'no'}
  }
  return(flag)
})

flagged <- lapply(seq_along(differ), function(i){
        if(differ[[i]] >= cutoff[[i]]){
        flag[[i]] <- 'yes'
      } else {flag[[i]] <- 'no'}
  return(flag[[i]])
})
...
png("./Figures/Cond Figures/Cond_val_varied_alpha_w_interp.png", units = "in", 
    width = 12, height = 12, res = 300, bg = "transparent")
