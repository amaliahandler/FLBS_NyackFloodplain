...
ha_do_list <- lapply(ha_do_list, function(x){
  if(x$sensor_number[1] == '01' | x$sensor_number[1] == '03' |
     x$sensor_number[1] == '04' | x$sensor_number[1] == '05' |
     x$sensor_number[1] == '07' | x$sensor_number[1] == '08'){
    x <- x[-which(x$do_temp <= 0), ]
     } else {x}
})

# Other known bad data based on the service log for the sensors
# HA12_05 was throwing bad DO data from 2013-06-15 until the cable was replaced on 2013-08-27 and had reasonable data starting at 12:00.
# During much of this period, the temperature data was constant and the DO concentration was NaN values.
# Remove all data from this period that had NaN DO concentration values. Also need to remove the two observations from 08-10 and 08-27 because these observations are anomalous.
ha_do_list[[5]] <- ha_do_list[[5]][-which(ha_do_list[[5]]$datetime >= 
                                           as.POSIXct('2013-06-15 00:00') & 
                              ha_do_list[[5]]$datetime <= as.POSIXct('2013-08-27 11:00') &
                              is.finite(ha_do_list[[5]]$do_conc) == FALSE),]

ha_do_list[[5]] <- ha_do_list[[5]][-which(ha_do_list[[5]]$datetime >= 
                                           as.POSIXct('2013-08-10 00:00') & 
                              ha_do_list[[5]]$datetime <= as.POSIXct('2013-08-27 11:00')),]

# Do the same manual process for sensor 09, which has a bunch of periods with uniform temperature data. The sensor service log indicates that this sensor was experiencing similar issues to sensor 5.
ha_do_list[[9]] <- ha_do_list[[9]][-which(ha_do_list[[9]]$datetime >= 
                                           as.POSIXct('2018-01-01 00:00') &
                              is.finite(ha_do_list[[9]]$do_conc) == FALSE),]

# Same problem for sensor 06 2019
ha_do_list[[6]] <- ha_do_list[[6]][-which(ha_do_list[[6]]$datetime >= 
                                           as.POSIXct('2019-01-01 00:00') &
                              is.finite(ha_do_list[[6]]$do_conc) == FALSE),]

# And for sensor 08 in 2019. I identified the last good observation as 2019-04-12 05:00.
ha_do_list[[8]] <- ha_do_list[[8]][-which(ha_do_list[[8]]$datetime > 
                                           as.POSIXct('2019-04-12 05:00') &
                              is.finite(ha_do_list[[8]]$do_conc) == FALSE),]
...
png("./Figures/DO_temp_anomaly_wo_interp.png", units = "in", 
